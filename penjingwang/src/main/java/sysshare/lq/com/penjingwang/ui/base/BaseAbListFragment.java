...
import sysshare.lq.com.penjingwang.widget.ContainerLayout;
...
public abstract class BaseAbListFragment<P extends BasePresenter<V>,V extends IView,T> extends BasePresenterFragment<P,V> implements LMRecyclerView.OnFooterAutoLoadMoreListener,IListDataView<T>{
    ...
    protected boolean isPreload;//是否已经预加载完成
    protected boolean isVisible;//是否可见
    protected boolean isFirst = true;//是否第一次加载
    protected boolean isEnableLazy = false;//是否懒加载
    ...
    protected abstract BaseListAdapter getmListAdapter();
}
